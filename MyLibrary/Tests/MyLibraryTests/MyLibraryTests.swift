import XCTest
import MyLibrary
// import WeatherService because of Weather Model
...
final class MyLibraryTests: XCTestCase {
    ...
    func testWeatherModule () async {
        
        // Given a jasonString is the same structure as the openWeather response
        let jasonString = """
            {
                "coord": {
                    "lon": 10.99,
        ...
        do{
            // When the jsonString can be decoded by the Weather model
            let jsonData = try JSONDecoder().decode(Weather.self, from: data)
            // Then get the correct temp from JSON. Test success.
